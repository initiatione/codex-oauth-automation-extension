...
- 当来源为 `SUB2API` 时，展示分组下方的 `账号优先级` 输入框；默认 `1`，仅允许保存大于等于 `1` 的整数，自动运行中会和其他运行关键配置一样锁定，最终用于 SUB2API 创建账号请求的 `priority`
- 在主延迟配置下方展示 `第6步等待` 输入框；默认 `20` 秒，允许 `0~300` 秒，配置会持久保存并在每次执行 Step 6 前由后台读取最新值，自动运行中会锁定
...
- HeroSMS 支持免费手机号复用：收到验证码并保存为 `freeReusablePhoneActivation` 后，后续轮次会先确认号码重新进入可收短信状态，再优先提交该号码；如果免费复用准备失败，不会静默购买或重新激活新号码，而是停下交给用户处理
...
- HeroSMS 免费复用激活记录 `freeReusablePhoneActivation`，用于保存已经收到验证码、需要跨轮次继续保留的号码
- localhost 回调地址
- 自动运行轮次信息
...
- CPA / SUB2API 配置，其中 SUB2API 包含地址、管理员账号密码、分组、默认代理和账号优先级 `sub2apiAccountPriority`
- Codex2API 配置
...
- 注册方式 `signupMethod`
- 接码开关、接码平台 `phoneSmsProvider`，以及 HeroSMS / 5sim 各自的 API Key、国家/地区、价格上限和平台扩展设置；HeroSMS 还保存免费复用开关 `freePhoneReuseEnabled / freePhoneReuseAutoEnabled`
- Step 6 注册成功稳定等待秒数 `step6RegistrationSuccessWaitSeconds`，默认 `20`，允许 `0~300`
- iCloud 相关偏好
- LuckMail API 配置
- 自动运行默认配置
...
1. 读取持久化配置 `step6RegistrationSuccessWaitSeconds`
2. 将配置规范化为 `0~300` 秒；默认 `20` 秒，`0` 表示不额外等待
3. 按当前配置等待注册成功状态和页面跳转稳定
4. 不再删除 ChatGPT / OpenAI cookies
5. 完成后进入后续登录链路
...
1. 按当前来源刷新 OAuth 地址：
   ...
   - SUB2API：打开后台并生成 OAuth 地址，同时把当前 `sub2apiAccountPriority` 随请求传给 SUB2API 页面脚本
...
4. 如果 HeroSMS 免费复用已开启且存在 `freeReusablePhoneActivation`，流程会优先让该激活进入等待短信状态并提交旧号码；准备失败或状态不可复用时不会自动购买新号，避免把免费复用失败变成隐形付费
5. 后台提交号码后，会按 `currentPhoneActivation.provider` 分发后续查码、完成、取消、ban、reuse：HeroSMS 走原 `getStatus / setStatus` 逻辑，5sim 走 `/v1/user/check|finish|cancel|ban|reuse`
6. 验证码被拒绝或长时间收不到短信时，会按平台决定重发、换号、ban/取消当前激活，必要时把自动流拉回 Step 7
7. 手机号验证完成后，如果认证页偶发进入资料页，内容脚本会复用 Step 5 的姓名生日填写逻辑补齐资料，再继续等待 OAuth 同意页
8. 手机号验证和可选资料页处理完成后，继续等待 OAuth 同意页出现
9. 尝试多轮点击“继续”
10. 一旦捕获 localhost callback，写入状态并完成步骤

补充：

- HeroSMS 和 5sim 都归入接码平台适配层；默认平台是 HeroSMS，5sim 产品码固定为 `openai`，operator 默认 `any`。
- 号码当前最多复用 3 次成功注册；超过上限后会清空可复用激活记录，下次重新申请新号码。
- HeroSMS 免费复用记录与普通付费复用池分开保存；当号码已经收到验证码并保存为免费复用记录时，成功收尾和异常清理都不会发送会释放该号码的终态 `setStatus(6)` 或 `setStatus(8)`，除非该记录已被明确淘汰。
...
4. 提交回调地址或等价的授权码交换请求；SUB2API 分支会继续携带 `sub2apiAccountPriority`，创建账号时写入 `priority`
