...
- sidepanel 另有“白嫖复用”开关，独立于 HeroSMS `reactivate`。开启后，后台只会在新申请号码首次成功收到短信验证码后，把该号码保存为 `freeReusablePhoneActivation`；该记录不会被普通 reset 或单账号任务完成自动清掉，只能通过侧栏清除按钮移除。
...
- 下次进入 `add-phone` 且“白嫖复用”开关仍开启时，后台会优先使用已保存的白嫖号码：内容脚本只选择国家并填入手机号，不点击提交按钮；随后后台清空当前 in-flight 激活状态、调用停止链路，并记录“开始手动复用手机，请到 SMS 上刷新”。这条手动交接路径发生在任何 HeroSMS `reactivate`、`getPrices`、`getNumber`、`getNumberV2` 调用之前，因此不会为第二次填号触发付费取号或付费重激活。
...
- 在 `phone-verification` 页等待短信验证码的整个过程中，后台都会让内容脚本只读探测页面是否出现“无法向此电话号码发送短信”、等价英文短信拒绝文案，或“尝试重新发送的次数过多。请稍后重试。”这类重发限流文案；一旦命中，会分别按 `resend_phone_banned` / `resend_throttled` 复用 Step 9 内部换号循环：释放当前尚未收到验证码的 HeroSMS 激活、清空当前激活状态、返回 `add-phone`，重新取号并提交，不重走 Step 7。
