...
- 展示一个单独的“接码”开关；开启后才展示 HeroSMS 的接码国家、API Key、复用与限流风险策略设置，用于 OAuth 登录链路命中手机号验证页时直接续跑手机验证
...
- iCloud 邮箱验证码结果缓存 `icloudVerificationResultState`：仅在当前 Step 4 / Step 8 验证会话内短暂保存已找到但尚未完全走完 transport 返回的验证码结果，用于处理“验证码已读到但响应超时丢失”的恢复场景
- localhost 回调地址
- 自动运行轮次信息
...
- 接码开关，以及 HeroSMS 的 API Key、默认国家、复用策略与限流大概率封号风险开关
- iCloud 相关偏好
- LuckMail API 配置
- 自动运行默认配置
...
- 邮箱页或注册页能在注入恢复后继续执行
- iCloud 邮箱页在恢复后重发 `POLL_EMAIL` 时，会保留一个最小响应窗口，避免 reload / reinject 后只剩约 1 秒响应时间而反复触发“页面通信异常”
- iCloud 邮箱页一旦在当前验证会话中找到有效验证码，会先把结果写入当前会话缓存；如果随后 `sendResponse` 通道超时，background 会优先消费该缓存结果，不再把这次命中误判成“继续刷新邮箱 / 继续重发验证码”
...
- `iCloud` 网页邮箱在 Step 4 / Step 8 轮询开始时会先刷新一次收件箱，并在记录旧邮件快照前检查最新匹配邮件的列表时间；若时间贴近本次 `filterAfterTimestamp` 且验证码未被排除，会直接使用该验证码，避免刚到达的邮件被旧快照误判为历史邮件。若时间过旧、不可解析或验证码已排除，则继续走原有“等待 1.5 秒 + 旧邮件快照 + 后续刷新轮询”逻辑。
...
4. 手机号验证完成后，会重新开启一段本地 OAuth 同意页 / “继续”按钮就绪等待窗口；手机号轮询、重发、换号消耗的时间不会继续占用这段等待
5. 尝试多轮点击“继续”
6. 一旦捕获 localhost callback，写入状态并完成步骤

补充：

- HeroSMS 号码当前最多复用 3 次成功注册；这条“平台同号重激活”路径会调用 HeroSMS `reactivate`，可能再次计费，超过上限后会清空可复用激活记录，下次重新申请新号码。
- sidepanel 另有“白嫖复用”开关，独立于 HeroSMS `reactivate`。开启后，后台会在新申请号码首次成功收到短信验证码后立刻把该号码保存为 `freeReusablePhoneActivation`，记录时机早于后续 OAuth 同意页点击与 Step 10 平台回调验证；该记录不会被普通 reset 或单账号任务完成自动清掉，可以通过侧栏清除按钮手动移除。
- `freeReusablePhoneActivation` 保存和清除都会向 sidepanel 广播 `DATA_UPDATED`，因此“白嫖号码”应当热刷新显示，不需要重载扩展或重新打开侧栏。
- 如果自动记录没有显示，sidepanel 的“白嫖号码”行提供手机号兜底输入框和“记录”按钮；只需填写 HeroSMS 页面上的手机号即可保存本地白嫖复用记录。只填手机号的记录可用于下一次手动填号复用；因为缺少 HeroSMS activation id，开启“自动白嫖复用”时会停止并提示不能自动激活，不会继续购买新号。
- 一旦 HeroSMS `getStatus / getStatusV2` 返回有效短信验证码，后台会把当前 `currentPhoneActivation` 标记为已收到验证码；从这一刻开始，后续验证码提交失败、回到 `add-phone`、换号、异常清理或用户停止，都只清本地运行态，不再向 HeroSMS 发送 `setStatus(8)` 取消该号码，避免破坏后续手动复用。
- 如果该验证码来自“白嫖复用”开关开启后的新申请号码，手机验证成功后也不会向 HeroSMS 发送 `setStatus(6)` 完成激活；扩展只保存 `freeReusablePhoneActivation` 并清理本地当前运行态，让该平台激活继续留给下次手动刷新短信使用。白嫖复用关闭时，成功号码仍按普通流程发送 `setStatus(6)` 完成激活。
- 下次进入 `add-phone` 且“白嫖复用”开关仍开启、但“自动白嫖复用”关闭时，后台会优先使用已保存的白嫖号码：内容脚本只选择国家并填入手机号，不点击提交按钮；随后后台清空当前 in-flight 激活状态、调用停止链路，并记录“开始手动复用手机，请到 SMS 上刷新”。这条手动交接路径发生在任何 HeroSMS `reactivate`、`getPrices`、`getNumber`、`getNumberV2` 调用之前，因此不会为第二次填号触发付费取号或付费重激活。
- 如果“白嫖复用”和“自动白嫖复用”同时开启，Step 9 会让白嫖记录优先于平台同号重激活：先对保存的 HeroSMS 激活调用 `setStatus(3)`，等待短暂延迟后再用原激活的 `getStatus / getStatusV2` 确认状态；`STATUS_WAIT_CODE / STATUS_WAIT_RETRY / STATUS_WAIT_RESEND` 以及这些状态后带 `:旧码或平台信息` 的后缀都只视为等待态，不会把后缀当作验证码提交。准备阶段如果仍看到旧的 `STATUS_OK` 或 V2 code，会再次 `setStatus(3)` 并重试确认，必须等到确认等待态后才自动提交该手机号。
- 自动白嫖复用的准备阶段有明确安全边界：如果保存的手机号在提交给 OpenAI 前无法确认进入等待短信状态，或 HeroSMS 准备接口失败，后台会停止当前自动流程并记录“本次不购买新 HeroSMS 号码”，不会继续调用平台同号 `reactivate`、`getPrices`、`getNumber` 或 `getNumberV2` 去消费新号。只有当保存手机号已经成功提交到 OpenAI 之后，页面明确返回拒号、封禁、或已开启高风险策略的重发限流，才按既有 Step 9 换号循环处理。
- 自动白嫖复用提交号码后复用现有短信等待、页面封号/限流探测、重发、换号、验证码提交和 OAuth 同意页续跑逻辑。自动复用成功后只递增 `freeReusablePhoneActivation.successfulUses`，未达到 `maxUses`（默认 3）会继续保留给后续注册；达到上限、HeroSMS 取消、OpenAI 拒号、页面出现短信封禁或高风险重发限流时，会按对应策略清掉本地白嫖记录，但不会因为清本地记录去调用 `setStatus(6)` 或 `setStatus(8)` 结束已经收到有效验证码的激活。
- 如果手机号验证提交成功后认证页已经到 `/sign-in-with-chatgpt/.../consent`，但“继续”按钮暂时还没出现在内容脚本状态中，Step 9 会继续按本地窗口等待并记录 URL、`consentPage`、`buttonFound`、`buttonEnabled`、`buttonText` 等诊断，不会因为此前短信等待耗时较长而立刻回退到 Step 7。
- 在 `phone-verification` 页等待短信验证码的整个过程中，后台都会让内容脚本只读探测页面是否出现“无法向此电话号码发送短信”或等价英文短信拒绝文案；一旦命中，会按 `resend_phone_banned` 复用 Step 9 内部换号循环：释放当前尚未收到验证码的 HeroSMS 激活、清空当前激活状态、返回 `add-phone`，重新取号并提交，不重走 Step 7。
- sidepanel 另有“限流大概率封号”开关，默认关闭且热生效。开启后，“尝试重新发送的次数过多。请稍后重试。”以及等价英文 `Tried to resend too many times. Please try again later.` 会被视为 `resend_throttled_high_risk_banned` 高概率不可用号码信号，在首轮短信等待、重发后的等待或重发点击返回错误时立即走 Step 9 换号循环；关闭时不会仅因这类限流文案触发高风险换号，只保留原有 SMS 等待、重发、超时换号等保守行为。
- “限流大概率封号”是吞吐优先的风险策略：限流文案可能只是 OpenAI 临时限制重发，不一定代表号码永久封禁。开启后可能误判并丢弃仍可恢复的号码，因此 UI 文案会明确提示“大概率”和“可能误判并丢弃仍可恢复的号码”。
...
1. 读取总轮数与模式
   - 侧边栏顶部的运行次数输入框负责选择总轮数；普通 provider 下可手动输入多轮数量，启动前会归一化为空值/非法值/小数/小于 1 的值
   - 如果当前生成方式是 `custom-pool`，或 `Mail = 自定义邮箱` 且配置了 `customMailProviderPool`，侧边栏会把总轮数锁定为对应池长度，避免轮次数量与邮箱池条目错位
