...
- 在 `phone-verification` 页点击“重新发送短信”后，如果页面出现“无法向此电话号码发送短信”或等价的英文短信拒绝文案，内容脚本会把它上报为 `PHONE_RESEND_BANNED_NUMBER`。后台会把它视为当前号码已被 OpenAI 拒绝发送短信，记录 `resend_phone_banned` 原因，并复用 Step 9 内部换号循环：释放当前尚未收到验证码的 HeroSMS 激活、清空当前激活状态、返回 `add-phone`，重新取号并提交，不重走 Step 7。
