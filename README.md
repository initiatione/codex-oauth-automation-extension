...
- 支持在 `SUB2API` 来源下配置账号优先级；默认 `1`，可填写大于等于 `1` 的整数，并在账号创建时写入 SUB2API 的 `priority`
- 支持 HeroSMS 免费手机号复用；号码收到验证码并保存后，后续轮次会优先尝试免费复用，避免在可复用场景下重复购买同一号码
- 支持在侧边栏配置 Step 6 注册成功后的稳定等待时间；默认 `20` 秒，可填写 `0~300` 秒，并在每次执行 Step 6 时读取最新配置
...
1. `来源` 选择 `SUB2API`
2. 填好 `SUB2API` 地址、登录邮箱、登录密码、分组名和账号优先级
...
- `账号优先级`：创建账号时写入 SUB2API 的 `priority`，默认 `1`；只能填写大于等于 `1` 的整数
...
账号优先级会跟随 OAuth 链路从后台传递到 SUB2API 页面内容脚本，并在最终创建 OpenAI 账号时写入请求体的 `priority` 字段。输入框允许实时编辑，但自动运行中会锁定，暂停或停止后再修改。

### `第6步等待`

侧边栏“延迟”配置下方提供 `第6步等待` 输入框，用于控制注册资料提交完成后等待注册成功状态稳定的时间：

- 默认 `20` 秒
- 可填写 `0~300` 秒，`0` 表示不额外等待
- 配置会持久保存；每次执行 Step 6 前后台都会读取最新值
- 输入期间会保留当前编辑值，避免后台状态同步把未保存的手动输入覆盖回旧值
- 已计划或自动运行中会锁定，暂停或停止后可继续修改
